...
import { type Inventory, type PlanningLevel, type ProductionPlan, type Step, StepType, type Order, type DeliveryStep, type Factory, type DestinationStep, type Train } from '../../types';
...
export const inventoryKeys = {
  ...
  stepChanges: (stepId: string) => [...inventoryKeys.all, 'stepChanges', stepId] as const,
  levelChanges: (levelId: number) => [...inventoryKeys.all, 'levelChanges', levelId] as const,
  atLevel: (levelNumber: number) => [...inventoryKeys.all, 'atLevel', levelNumber] as const,
};
 ...
 */
const calculateFactoryStepChanges = (step: Step, factories: Record<string, any>): Map<string, number> => {
  const changes: Map<string, number> = new Map();
 ...
 */
const calculateDestinationStepChanges = (step: DestinationStep, trains: Record<string, Train>): Map<string, number> => {
  const changes: Map<string, number> = new Map();
 ...
 */
const calculateDeliveryStepChanges = (step: DeliveryStep, trains: Record<string, Train>): Map<string, number> => {
  const changes: Map<string, number> = new Map();
 ...
 */
const calculateSubmitStepChanges = (step: Step, order?: Order): Map<string, number> => {
  const changes: Map<string, number> = new Map();
  ...
  const resourceRequirement = order.resources.find(r => r.resourceId === step.resourceId);
  if (!resourceRequirement) {
    console.warn(`Resource requirement not found for submit step: ${step.id}, resourceId: ${step.resourceId}`);
    return changes;
  }
 ...
 */
export const useLevelInventoryChanges = (level: PlanningLevel, orders?: Order[]) => {
  const { data: factories } = useFactories();
  const { data: trains } = useTrains();

  return useQuery({
    queryKey: inventoryKeys.levelChanges(level.level),
    queryFn: () => {
      if (!factories || !trains) return new Map();
      ...
      level.steps.forEach(step => {
        ...
        calculateStepInventoryChange(step, order, factories, trains).forEach((value, key) => {
 ...
 */
export const useInventoryAtLevel = (productionPlan: ProductionPlan, levelNumber: number) => {
